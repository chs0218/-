...
int main(int argc, char** argv)
{
	...
	vShader = MakeVertexShader("vertex_alpha.glsl", 0);
	fShader = MakeFragmentShader("fragment_alpha.glsl", 0);

	s_program_alpha = glCreateProgram();
	glAttachShader(s_program_alpha, vShader);
	glAttachShader(s_program_alpha, fShader);
	glLinkProgram(s_program_alpha);
	checkCompileErrors(s_program_alpha, "PROGRAM2");
	InitBuffer();
...
void InitBuffer()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(Alphavertices), Alphavertices, GL_STATIC_DRAW);
...
void DrawShapes(glm::mat4 view, glm::mat4 projection, GLfloat ResultX, GLfloat ResultZ)
{
	...
	// 불투명 도형
	glUseProgram(s_program_alpha);
	glBindVertexArray(AlphaVAO);

	viewLocation = glGetUniformLocation(s_program_alpha, "viewTransform");
	projectionLocation = glGetUniformLocation(s_program_alpha, "projectionTransform");
	lightPosLocation = glGetUniformLocation(s_program_alpha, "lightPos");
	lightColorLocation = glGetUniformLocation(s_program_alpha, "lightColor"); //--- lightColor 값 전달: (1.0, 1.0, 1.0) 백색
	viewPosition = glGetUniformLocation(s_program_alpha, "viewPos");	//--- 카메라 위치
	modelLocation = glGetUniformLocation(s_program_alpha, "modelTransform");
	ambientLocation = glGetUniformLocation(s_program_alpha, "ambient");
	objColorLocation = glGetUniformLocation(s_program_alpha, "objectColor"); //--- object Color값 전달: (1.0, 0.5, 0.3)의 색
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);
	glUniformMatrix4fv(projectionLocation, 1, GL_FALSE, &projection[0][0]);
	glUniform3f(lightPosLocation, ResultX, lightY, ResultZ);
	glUniform3f(lightColorLocation, lightColor[0], lightColor[1], lightColor[2]);
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		transformMatrix = glm::translate(transformMatrix, glm::vec3(obstacle[i].getX(), 2.0 * BOXSIZE, obstacle[i].getZ()));
		transformMatrix = glm::scale(transformMatrix, glm::vec3(1.0, 4.0, 1.0));
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(transformMatrix));
...
void Drawlight(glm::mat4 view, glm::mat4 projection, GLfloat ResultX, GLfloat ResultZ)
{
	...
	lightMatrix = glm::translate(lightMatrix, glm::vec3(ResultX * 1.1, lightY * 1.1, ResultZ * 1.1));
...
void RotateCamera()
{
	GLfloat tmpX = cameraPos.z * glm::sin(glm::radians(1.0)) + cameraPos.x * glm::cos(glm::radians(1.0));
	GLfloat tmpZ = cameraPos.z * glm::cos(glm::radians(1.0)) - cameraPos.x * glm::sin(glm::radians(1.0));
